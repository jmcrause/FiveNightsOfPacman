...
NightTwo::NightTwo(QWidget *parent) :
    QWidget(parent)
{
    ...
    bon->setStyleSheet("background-color: red;");
    bon->Bonnie();
    ...
    fox = new Animatronics(this);
    fox->Foxy();
    fox->move((5*50),(3*50));
    fox->show();

    redBlock = new QLabel(this);
    redBlock->setFixedSize(50,50);
    redBlock->move(5*50, 3*50);
    redBlock->show();
    ...
    foxMove = new QTimer(this);
    foxMove->setInterval(3000);
    connect(foxMove, SIGNAL(timeout()), this, SLOT(FoxyMove()));
    foxMove->start();
    intro = Phonon::createPlayer(Phonon::MusicCategory, Phonon::MediaSource("D:/Werk/3de jaar/Semester 1/EERI 316/Menu/Audio/Night2.wav"));
    intro->play();
...
void NightTwo::keyPressEvent(QKeyEvent *e)
{
...
void NightTwo::trackPac()
{
    ...
    {
        curtain->show();
        redBlock->setStyleSheet("background-color: rgb(150, 0, 0);");
    }
     ...
     {
         curtain->hide();
         redBlock->setStyleSheet("background-color: black;");
         for(int k = 0; k<=9; k++)
         {
             tables[k]->hide();
         }
...
void NightTwo::BonnieRun()
{
    chaseB = false;
    cB->stop();
}

void NightTwo::ChikaRun()
{
    chaseC = false;
    cC->stop();
}

void NightTwo::FreddyRun()
{
    chaseF = false;
    cF->stop();
}

void NightTwo::FoxyMove()
{
    if(f <= 50)
    {
        fox->move(fox->x() + 1, fox->y());
    ...
    if(f == 48)
    {
        foxSong = Phonon::createPlayer(Phonon::MusicCategory, Phonon::MediaSource("D:/Werk/3de jaar/Semester 1/EERI 316/Menu/Audio/Fox.wma"));
        foxSong->play();
    }
    if(f > 50)
    {
        foxMove->setInterval(3);

        if(pac->x()< fox->x())
        {
            fox->move(pac->x()+ lenxFox, fox->y());
            lenxFox = lenxFox -1;
        }
        //--------------------top--------------------------
        if(pac->y() < fox->y())
        {
            fox->move(fox->x(), pac->y()+ lenyFox);
            lenyFox = lenyFox -1;
        }
        //-------------------Bottom-------------------------
        if(pac->y() > fox->y())
        ...
        if(pac->x()> fox->x())
        {
            fox->move(fox->x()+ lenxFox, fox->y());
            lenxFox = lenxFox + 1;
        }
        //----------------------Caught------------------------------
        if(fox->x() == pac->x() && fox->y() == pac->y())
        {
             music->stop();
             foxSong->stop();
             this->close();
